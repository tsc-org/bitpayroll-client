...
const SignUp = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const query = accountType === AccountType.EMP ? endpoints.REGISTER_EMPLOYEE() : endpoints.REGISTER_ORG()
    const body = {
        email: data.email,
    ...
    <>
      ...
      >
        ...
        <Center position="relative" pt={6}>
            ...
            <HStack spacing={8}>
                <Button onClick={() => switchPage(0)} size="no-size" variant={!data.page ? "page-switch-active" : "page-switch-inactive"}>1</Button>
                <Button onClick={() => switchPage(1)} size="no-size" variant={data.page ? "page-switch-active" : "page-switch-inactive"}>2</Button>
            </HStack>
        </Center>
        <Box mt={14} className={styles.page_container}>
            <div className={`${styles.extended_section} ${data.page ? styles.extended_section_two : styles.extended_section_one}`}>
                <div className={styles.page_one}>
                    <Text textAlign="left" fontWeight="500" mb={8}>
                    How would you like to sign up?
                    </Text>
                    <RadioGroup mb={"100px"} onChange={handleAccountType} value={accountType}>
                        <Stack spacing={7}>
                            <Radio size="sm" value={AccountType.ORG}>As an organization</Radio>
                            <Radio size="sm" value={AccountType.EMP}>As an employee</Radio>
                        </Stack>
                    </RadioGroup>
                    <Button
                            ...
                            <InputGroup size="lg">
                                ...
                                <InputRightElement
                                    ...
                                    <IconButton
                                        ...
                                        icon={passwordVisibility.password ? <FiEye /> : <FiEyeOff />}
                                        fontSize="20px"
                                        variant={"flat"}
                                        color="grey.200"
                            ...
                            <FormControl isInvalid={validateConfirmPassword()}>
                                <InputGroup size="lg">
                                    ...
                                    <InputRightElement
                                        ...
                                        <IconButton
                                            aria-label="show/hide password"
                                            icon={passwordVisibility.confirmPassword ? <FiEye /> : <FiEyeOff />}
                                            fontSize="20px"
                                            variant={"flat"}
                                            color="grey.200"
                                ...
                                {validateConfirmPassword() && <FormErrorMessage>Passwords don't match</FormErrorMessage>}
                            </FormControl>
                        </Stack>
                        <Button
